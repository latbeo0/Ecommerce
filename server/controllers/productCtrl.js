...
const productCtrl = {
  ...
  test: async (req, res) => {
    try {
      ...
      const selectedField = ["categories", "collections", "colors", "sizes"];
      ...
      for (const product of listProductsTemp) {
        ...
        const { productName, productDescription, vCategory, vCollection } =
          productMaster;
      ...
      const checkFilter = (product) => {
        ...
        for (const key in newQueryObject) {
          if (key === "categories" || key === "collections") {
            ...
            if (!newQueryObject[key].includes(product.color.valueColor)) {
              isPassed = false;
            }
          } else if (key === "sizes") {
      ...
      // Lọc sản phẩm
      const filter = listProducts.filter((product) => checkFilter(product));
  ...
  getProductById: async (req, res) => {
    try {
      ...
      const { productName, productDescription, collectCode } = productMaster;
    ...
    try {
      ...
      for (const idMaster of idProductsMaster) {
        ...
        for (const product of products) {
          if (product._id.toString() !== id) listRelatedProducts.push(product);
        }
