...
const AvatarFormatter = ({ value }) => (
  <ImageList variant="masonry" cols={1} sx={{ width: 160 }}>
    <ImageListItem sx={{ width: 50, height: 50 }}>
      <img src={`${value}`} srcSet={`${value}`} alt={`avatar`} loading="lazy" />
    </ImageListItem>
  </ImageList>
);
const ColorsFormatter = ({ value }) => (
  <ListColorContainer>
    {value?.map((item, index) => (
      <ColorCard
        key={`${item.valueColor}_Number${index}`}
...
const ColorFormatter = ({ value }) => (
  <ListColorContainer>
    <ColorCard color={value?.valueColor} />
  </ListColorContainer>
);
const CurrencyFormatter = ({ value }) => (
  <CurrencyContainer>{formatCurrency(value, "VND")}</CurrencyContainer>
);
...
export const AvatarTypeProvider = (props) => (
  <DataTypeProvider
...
export const ColorTypeProvider = (props) => (
  <DataTypeProvider
